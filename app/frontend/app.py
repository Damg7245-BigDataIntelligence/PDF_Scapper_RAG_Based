...
with st.sidebar:
    ...
    rag_option = st.selectbox(
        "RAG Option",
        options=rag_options,
        index=0,  # Start at the placeholder
        key="rag_option"
    )
...
# Show the content if a document is selected
if st.session_state.selected_document and st.session_state.document_content:
    ...
    # Tabs for content, summarization, Q&A
    ...
    1. **Choose your RAG Option** (Without VectorDB, ChromaDB, Pinecone).
    2. **Pick a Chunking Strategy** (Fixed-Size, Semantic, or Sentence-Based).
    3. **Select an AI Model** (Zephyr or Gemini).
    4. **Upload a PDF** or select an existing processed document.
    5. **View the document** in the Document Content tab.
    6. **Generate a summary** in the Summarize tab.
    7. **Ask questions** in the Ask Questions tab.
